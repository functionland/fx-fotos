...
export const prepareLayout = (
  ...
  let lastTimestampObj = timestampToDate(lastTimestamp, [
    ...sortConditions,
    'year',
  ]);

  let lastYear = {
    day: lastTimestampObj.year || '',
    month: lastTimestampObj.year || '',
  };
  ...
  for (let i = 0; i < newMedias.length; i++) {
    ...
    let mediaTimestampObj = timestampToDate(newMedias[i].modificationTime, [
      ...sortConditions,
      'year',
    ]);
    ...
    if (
      now.getDate() === media.getDate() &&
      now.getFullYear() === media.getFullYear()
    ) {
    ...
    for (let j = 0; j < sortConditions.length; j++) {
      ...
      if (
        mediaTimestampObj[sortCondition_j] !==
          lastTimestampObj[sortCondition_j] ||
        lastYear[sortCondition_j] !== mediaTimestampYear
      ) {
        ...
        let lastHeaderIndex = [...headerIndexes]
          .reverse()
          .findIndex(
            (headerIndex) => headerIndex.sortCondition === sortCondition_j,
          );
        if (lastHeaderIndex > -1) {
          headerIndexes[headerIndexLength - 1 - lastHeaderIndex].count =
            count[sortCondition_j];
        }
