    ...
    for(let i=0; i<newMedias.length; i++){
      ...
      for(let j=0;j<sortConditions.length;j++){
        ...
        if(mediaTimestampObj[sortCondition_j] !== lastTimestampObj[sortCondition_j] || lastYear[sortCondition_j] !== mediaTimestampYear){
          ...
          layout.push({value:mediaTimestampObj[sortCondition_j], sortCondition: sortCondition_j, index:-1, deleted:false, id: (newMedias[i].modificationTime+''+j)});
      ...
      layout.push({value:newMedias[i], sortCondition: '', index: i+lastIndex, deleted:false, id: newMedias[i].id});
