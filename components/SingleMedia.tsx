...
import {BaseScrollView, DataProvider, RecyclerListView} from 'recyclerlistview';
...
const SingleMedia: React.FC<Props> = (props) => {
  ...
  const viewPosition = useRef(
    new Animated.ValueXY(props.imagePosition),
  ).current;
  ...
  const imageScale: Animated.AnimatedInterpolation = Animated.multiply(
    _baseImageScale,
    _pinchScale,
  ).interpolate({
    inputRange: [0, 1, 4],
  ...
  const hideModalAnimation = (duration: number = 400) => {
    ...
    let thumbnailPositionMinusSingleImagePosition = {
      x:
        props.imagePosition.x -
        ((SCREEN_WIDTH / (props.numColumns * imageDimensions.width)) *
          (SCREEN_WIDTH - imageDimensions.width)) /
          2,
      y:
        props.imagePosition.y -
        ((SCREEN_WIDTH / (props.numColumns * imageDimensions.height)) *
          (SCREEN_HEIGHT - imageDimensions.height)) /
          2,
    };
    Animated.parallel([
      Animated.timing(viewPosition, {
      ...
      Animated.timing(viewScale, {
        toValue: {
          x: SCREEN_WIDTH / (props.numColumns * imageDimensions.width),
          y: SCREEN_WIDTH / (props.numColumns * imageDimensions.height),
        },
        duration: duration,
        useNativeDriver: true,
      }),
    ...
    if (
      ...
      if (
        ...
        ((event.nativeEvent.translationX /
          (event.nativeEvent.translationY + 1) >
          0 &&
          event.nativeEvent.translationX /
            (event.nativeEvent.translationY + 1) <
            0.6) ||
      ...
      if (
        event.nativeEvent.velocityX / (event.nativeEvent.velocityY + 1) > 0.6 ||
        event.nativeEvent.translationX / (event.nativeEvent.translationY + 1) <
          -0.6
      ) {
            ...
            transform: [
              {
                translateY: Animated.subtract(
                  Animated.add(viewPosition.y, translationYvsX),
              ...
              transform: [
                ...
                },
                {
                  translateX: Animated.divide(
                    Animated.subtract(
                      Animated.multiply(viewScale.x, SCREEN_WIDTH),
                      SCREEN_WIDTH,
                    ),
                ...
                },
                {
                  translateY: Animated.divide(
                    Animated.subtract(
                      Animated.multiply(viewScale.y, SCREEN_HEIGHT),
                      SCREEN_HEIGHT,
                    ),
                ...
                <Animated.View
                  ...
                  <RecyclerListView
                    ...
                    extendedState={{
                      ...
                      activeIndex: props.singleMediaIndex,
                    }}
                    style={{
                      width: SCREEN_WIDTH,
                      height: SCREEN_HEIGHT,
                    ...
                    rowRenderer={(
                      ...
                      <Media
                        imageHeight={
                          calcImageDimension(item, SCREEN_HEIGHT, SCREEN_WIDTH)
                            .height
                        }
                        imageWidth={
                          calcImageDimension(item, SCREEN_HEIGHT, SCREEN_WIDTH)
